...
window.onload = function () {
    ...
    var tags = ["BanhNgot", "BanhSinhNhat", "BanhMi", "BanhMan"];
...
function phanTich_URL_chiTietSanPham() {
    ...
    // Đổi title
    document.title = nameProduct + ' - NCB';
    ...
    var s = addThongSo('Khu vực', sanPhamHienTai.detail.area);
    s += addThongSo('Phân loại', sanPhamHienTai.detail.tags);
    s += addThongSo('Mô tả', sanPhamHienTai.detail.describe);
...
function getDetailPromo(sp) {
    switch (sp.promo.name) {
        ...
        case 'moiramat':
            return `Khách hàng sẽ được nhân viên tặng thêm nhiều niềm vui khi dùng thử sản phẩm mới tại cửa hàng.`;

        default:
            return `Mời quý khách đến với CALEM để được hưởng thêm nhiều ưu đãi và khuyến mãi!`;
    }
}
...
function suggestion(){
    ...
    .map(sanPham => {
        ...
        let giaTienGanGiong = Math.abs(giaSanPham - giaSanPhamHienTai) < 100000;
